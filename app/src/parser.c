...
static bool parsePortPin(Token token, uint32_t *port, uint32_t *pin) {
    ...
    *port = GPIOA;
    ...
    for (char port_char = PORTA_STM32F411RE;
         port_char < (PORTe_STM32F411RE + 1); port_char++) {
 ...
 */
static bool inputOutput(BoardController *bc, TokenVector *vec,
                        OpCode input_output) {
    size_t vec_size = sizeTokenVector(vec);
    // Ignore end of line token.
    if (vec_size - 1 != INPUT_OUTPUT_MAX_ARGS) {
        printf(
            "> Parse Error: Invalid input format, use \"input <port pin> "
    ...
    for (size_t i = 1; i < vec_size; i++) {
        ...
        case TOKEN_GPIO_PULLDOWN: {
            ...
            if (!pupd_set) {
                pupd = (current_token.type == TOKEN_GPIO_NORESISTOR)
                           ? GPIO_PUPD_NONE
                       : (current_token.type == TOKEN_GPIO_PULLUP)
                           ? GPIO_PUPD_PULLUP
                           : GPIO_PUPD_PULLDOWN;
                pupd_set = true;
            } else {
 ...
 */
static bool setResetToggleRead(BoardController *bc, TokenVector *vec,
                               OpCode operation) {
        ...
        if (current_token.type == TOKEN_EOL) // Ignore EOL token.
        {
            ...
            if (digitalPinExists(bc, port, pin)) {
                ...
                case OP_READ: {
                    ...
                    printf("> READ %.*s = %u\r\n", current_token.length,
                           current_token.start, read_response);
                    break;
                }
                default: {
